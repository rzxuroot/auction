(function(a, b) {
    var c = function() {
        function d(b, d) {
            "dot" == b ? (d = '<ol class="dots">', a.each(c.li,
            function(a) {
                d += '<li class="' + (a == c.i ? b + " active": b) + '">' + ++a + "</li>"
            }), d += "</ol>") : (d = '<div class="', d = d + b + 's">' + d + b + ' prev">' + c.o.prev + "</div>" + d + b + ' next">' + c.o.next + "</div></div>"),
        ...
        c.init = function(b, e) {
            ...
            c.max = [0 | b.outerWidth(), 0 | b.outerHeight()],
            c.li = c.ul.find(c.o.item).each(function() {
                var d = a(this),
                e = d.outerWidth(),
                f = d.outerHeight();
                e > c.max[0] && (c.max[0] = e),
            ...
            setTimeout(function() {
                0 | e.delay && (c.play(), e.pause && b.on("mouseover mouseout",
                function(a) {
                    c.stop(),
                    "mouseout" == a.type && c.play()
                }))
            },
            0 | e.init),
            e.keys && a(document).keydown(function(a) {
                var b = a.which;
                37 == b ? c.prev() : 39 == b ? c.next() : 27 == b && c.stop()
            }),
            e.dots && d("dot"),
            e.arrows && d("arrow"),
            e.fluid && a(window).resize(function() {
                ...
                c.r = setTimeout(function() {
                    ...
                    d = b.outerWidth();
                    f.css(a),
                    a.width = Math.min(Math.round(100 * (d / b.parent().width())), 100) + "%",
                    b.css(a)
                },
                50)
            }).resize(),
            (a.event.special.swipe || a.Event("swipe")) && b.on("swipeleft swiperight swipeLeft swipeRight",
            function(a) {
        ...
        c.to = function(d, e) {
            ...
            if (k.length && !(0 > d) || f.loop != b) {
				...
				if(c.i==3 && d==0){
					...
					function() {
						c.i = d,
						a.isFunction(f.complete) && !e && f.complete(g)
					}))
				}else{
					 h.queue("fx").length || (g.find(".dot").eq(d).addClass("active").siblings().removeClass("active"), g.animate(m, l) && h.animate(a.extend({
						left: "-"+d+"00%"
